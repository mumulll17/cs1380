...
test("(5 pts) (scenario) use mem.reconf", (done) => {
  ...
  */
  ...
  const keysAndItems = [
    { key: "n", item: { first: "Josiah", last: "Carberry" } },
  ];
  ...
  distribution.local.groups.put(config, mygroupGroup, (e, v) => {
    ...
    console.log("Put successful:", keysAndItems[0].key);
    distribution.mygroup.mem.put(
      keysAndItems[0].item,
      keysAndItems[0].key,
      (e, v) => {
        if (e) return done(new Error(`put() failed: ${e.message}`));
  ...
  const checkPlacement = (e, v) => {
    const messages = [[{ key: keysAndItems[0].key, gid: "mygroup" }]];
